...
@class YXRecorderManager;
@protocol YXRecorderManagerDelegate <NSObject>

- (void)recorderManager:(YXRecorderManager *)manager didReciveSampleBuffer:(CMSampleBufferRef _Nullable)buffer orImageBuffer:(CVPixelBufferRef _Nullable)imageBuffer type:(UUSampleBufferType)type;

@end

/// 录制的类
@interface YXRecorderManager : NSObject
@property (nonatomic, weak) id<YXRecorderManagerDelegate> delegate;
